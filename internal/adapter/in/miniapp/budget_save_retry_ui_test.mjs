...
function harness(kind,policyMode=false){
 ...
 register:s=>{screen=s;},currentScreen:()=>current,go:id=>{current=id;},addStrings(){},T:k=>k,sub:k=>k,fmtMoney:String,haptic:{bad(){},tap(){},ok(){}},toast(){},invalidate(){},fundingHTML:'',
...
for(const mode of ['configuration','funding','policy']){
 ...
 assert.ok(html.indexOf(`id="${prefix}retry"`)<html.indexOf(`id="${prefix}fields"`),'retry must be outside the disabled fieldset');
 ...
 assert.equal(h.field(prefix+'fields').disabled,true,'freeze inputs while request is in flight');assert.equal(h.field(prefix+'retry').disabled,true);
 ...
 assert.equal(h.field(prefix+'fields').disabled,true,'uncertain outcome must keep fields frozen');assert.equal(h.field(prefix+'retry').hidden,false);assert.equal(h.field(prefix+'retry').disabled,false);
 ...
 h.setPost(async()=>response());await h.retry();assert.equal(h.calls.at(-1).body,initial.body,'retry must be byte-for-byte identical');assert.equal(h.getKey(),1);assert.equal(h.field(prefix+'retry').hidden,true);
 ...
 assert.equal(h.field(prefix+'fields').disabled,false,'definitive validation refusal unlocks edit');assert.equal(h.field(prefix+'retry').hidden,true);assert.equal(h.field(prefix+'reload').disabled,false);
