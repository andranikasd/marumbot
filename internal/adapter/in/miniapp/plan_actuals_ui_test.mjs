...
vm.runInNewContext(source,env);field('activity-analysis').open=true;field('activity-filter').value='all';field('activity-month').value='2026-09';response=async()=>({comparisons:[comparison]});await screen.onShow();
...
field('activity-analysis').open=false;calls.length=0;await screen.onShow();assert.deepEqual(calls,['api/activity'],'opening payment history should not calculate hidden financial reports');
