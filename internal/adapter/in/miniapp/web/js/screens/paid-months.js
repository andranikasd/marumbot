...
async function load(s,discard=false){
 ...
 try{
  ...
  s.doc=d;s.values={month:d.today.slice(0,7),balance:String(d.balance_major),payment:d.payment_major==null?'':String(d.payment_major),confirmed:false};
...
function changed(){
 ...
 active.values={month:$('pm-month').value,balance:$('pm-balance').value,payment:$('pm-payment').value,confirmed:$('pm-confirm').checked};active.dirty=true;
...
async function save(event){
 ...
 if(!s.pending){
  ...
  try{
   ...
   s.pending={month:s.values.month,as_of:s.doc.today,balance_major:balance,payment_major:payment,confirmed:true};
 ...
 try{
  ...
  if(active===s&&currentScreen()==='paid-months')go('loan',{id:s.id});
...
register({id:'paid-months',parent:'loans',titleKey:'pm.title',html:`
 ...
 <button class="alink" type="button" data-go="plan-start" data-i18n="ps.title"></button></div>`,
