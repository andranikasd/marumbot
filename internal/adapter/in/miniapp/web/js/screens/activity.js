...
addStrings({'payment.status.reconciled':'Մնացորդները թարմացված են','activity.reconcile':'Ստուգել բանկի մնացորդները'},{'payment.status.reconciled':'Balances updated','activity.reconcile':'Check bank balances'});
...
addStrings({
 'activity.intro':'Այստեղ ձեր գրանցած վճարումներն են։ «Ստուգել բանկի մնացորդները» կոճակը կօգնի թարմացնել պլանը վճարումից հետո։',
 'activity.emptyNext':'Վճարո՞ւմ եք կատարել։ Բացեք վարկը և ընտրեք «Գրանցել վճարումը»։',
 'activity.chooseLoan':'Ընտրել վարկը', 'activity.details':'Ամսվա ամփոփում և պլանի համեմատություն'
},{
 'activity.intro':'These are the payments you recorded. Use “Check bank balances” to update your plan after a payment.',
 'activity.emptyNext':'Made a payment? Open the loan and choose “Record a payment”.',
 'activity.chooseLoan':'Choose a loan', 'activity.details':'Monthly totals and plan comparison'
});
let facts=[],busy=false,nextCursor="";
function loadVisibleAnalysis(){return document.getElementById('activity-analysis').open?Promise.all([loadActuals(),loadPlanActuals()]):Promise.resolve();}
function render(){
 ...
 }).join('')||`<div class="state"><b>${esc(T('activity.empty'))}</b><p>${esc(T('activity.emptyNext'))}</p><button class="cta" data-go="loans">${esc(T('activity.chooseLoan'))}</button></div>`;
}
...
register({id:'activity',icon:icon('document'),labelKey:'tab.activity',html:`<p class="hint" data-i18n="activity.intro"></p><div class="card field"><label for="activity-filter" data-i18n="activity.latest"></label><select id="activity-filter"><option value="all" data-i18n="activity.all"></option><option value="payments" data-i18n="activity.payments"></option></select></div><p id="activity-error" class="error" role="alert"></p><button class="alink" type="button" id="activity-retry" data-i18n="retry" hidden></button><div id="activity-list" class="stack"></div><button id="activity-more" class="alink" data-i18n="activity.more" hidden></button><details id="activity-analysis" class="fold"><summary data-i18n="activity.details"></summary><div class="fold-body stack"><div class="card stack"><label for="activity-month" data-i18n="activity.actuals"></label><input id="activity-month" type="month"><p class="hint" data-i18n="activity.basis"></p><div id="activity-actuals" class="stack"></div></div><div class="card stack"><b data-i18n="progress.title"></b><p class="hint" data-i18n="progress.help"></p><div id="activity-plan-actuals" class="stack"></div></div></div></details>`,onMount(el){
 ...
 el.addEventListener('click',async e=>{
  ...
  try{
   ...
   invalidate('api/');await Promise.all([load(),loadVisibleAnalysis()]);
  }catch(err){document.getElementById('activity-error').textContent=err.message||T('err.save');}
  finally{busy=false;button.disabled=false;}
 });
},onLanguage(){render();loadVisibleAnalysis();},async onShow(){document.getElementById('activity-error').textContent='';document.getElementById('activity-retry').hidden=true;try{await Promise.all([load(),loadVisibleAnalysis()]);}catch{document.getElementById('activity-list').textContent='';document.getElementById('activity-error').textContent=T('err.load');document.getElementById('activity-retry').hidden=false;}}});
