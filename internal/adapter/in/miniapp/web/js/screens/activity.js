...
async function loadActuals(){
 ...
 }catch{if(generation===actualsGeneration)el.textContent=T('activity.actuals.error');}
}
...
async function loadPlanActuals(){
 const generation=++progressGeneration,el=document.getElementById('activity-plan-actuals');el.textContent='';
 try{
  const month=document.getElementById('activity-month').value;
 ...
 }catch{if(generation===progressGeneration)el.textContent=T('progress.error');}
}
...
},async onShow(){document.getElementById('activity-error').textContent='';try{await Promise.all([load(),loadActuals(),loadPlanActuals()]);}catch{document.getElementById('activity-error').textContent=T('err.load');}}});
