...
const exact=await read('screens/projection-format.js');
...
function setup(){
 ...
 vm.createContext(env);vm.runInContext(exact,env);vm.runInContext(money,env);vm.runInContext(mutation,env);vm.runInContext(source,env);screen.onMount();return {node,calls,screen};
}
...
wait=s.node('pm-retry').onclick();sent=s.calls.shift();assert.deepEqual(sent.options,original);sent.resolve({ok:true});await wait;assert.equal(s.calls.shift().go,'simple-plan');
...
const interest=setup();wait=interest.screen.onShow(null,{id:'loan1'});interest.calls.shift().resolve({ok:true,json:async()=>doc});await wait;interest.node('pm-confirm').checked=true;interest.node('pm-accrued').value='0';wait=interest.node('pm-form').submit();sent=interest.calls.shift();assert.equal(JSON.parse(sent.options.body).accrued_interest_major,'0.00');sent.resolve({ok:true});await wait;
